    ...
    return new Promise(function(resolve, reject){
        ...
        Cardapio.remove({ usuario_id: consulta.cliente }, function(err) {
            if (!err) {
                ...
                if(dadosConsulta.relatorioCompleto){
                    var urlCompletaRelCompleto = serverInfo.serverUrl + '/uploads/relComp'+nomeArquivo+'.pdf';
                    consulta.linkRelatorio = urlCompletaRelCompleto;
                    var base64Foto = dadosConsulta.relatorioCompleto.value;
                    var urlReqExame = './uploads/relComp'+nomeArquivo+'.pdf';

                    console.log('chegou até aqui');
                    require("fs").writeFile(urlReqExame, base64Foto, {encoding: 'base64'}, function(err) {
                        if (err){
                            console.log('erro182p3sadasdasdas7');
                            reject({"status":false, "message":"Erro ao salvar a relatorioCompleto", "error": err});
                        }
                    });
                }
